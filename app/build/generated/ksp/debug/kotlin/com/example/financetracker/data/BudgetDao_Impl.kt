  ...
  init {
    ...
    this.__insertAdapterOfBudget = object : EntityInsertAdapter<Budget>() {
      protected override fun createQuery(): String =
          "INSERT OR REPLACE INTO `budgets` (`id`,`categoryId`,`amount`,`month`,`year`) VALUES (nullif(?, 0),?,?,?,?)"
    ...
    this.__updateAdapterOfBudget = object : EntityDeleteOrUpdateAdapter<Budget>() {
      protected override fun createQuery(): String =
          "UPDATE OR ABORT `budgets` SET `id` = ?,`categoryId` = ?,`amount` = ?,`month` = ?,`year` = ? WHERE `id` = ?"
    ...
    return createFlow(__db, false, arrayOf("budgets")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = Budget(_tmpId,_tmpCategoryId,_tmpAmount,_tmpMonth,_tmpYear)
          _result.add(_item)
        }
        _result
      } finally {
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          val _tmpYear: Int
          _tmpYear = _stmt.getLong(_columnIndexOfYear).toInt()
          _result = Budget(_tmpId,_tmpCategoryId,_tmpAmount,_tmpMonth,_tmpYear)
        } else {
          _result = null
        }
        _result
